...
public class Genertor003L04 : MonoBehaviour
{
    ...
    void Update()
    {
        if (IsActive && Input.GetKeyUp(KeyCode.F))
        {
            ...
            if (state % 2 == 0)
            {
                ...
                target5.position = new Vector3(target5.position.x, 5.5f, target5.position.z);
                mate.material.color = Color.white;
            }
            else if (state % 2 != 0)
            {
                ...
                target5.position = new Vector3(target5.position.x, -0.5f, target5.position.z);
